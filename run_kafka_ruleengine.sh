...
if [ $# -eq 0 ]
then
	echo "./run_kafka_ruleengine.sh - process messages from kafka using the JaRE ruleengine"
	echo "usage: ./run_kafka_ruleengine.sh [ruleengine project zip file] [ruleengine properties file] [kafka consumer properties file] [kafka producer properties file]"
	echo "   or: ./run_kafka_ruleengine.sh [ruleengine project zip file]"
	echo
	echo "       the last three arguments are optional, if the files are located in the same folder as this script"
	echo
else
	...
	then
		...
		echo "error: ruleengine project file [${ruleengine_project_file}] not found."
	fi

	if [ ! -f "${properties_file}" ]
	then
		errorRuleEnginePropertiesFile=true
		echo "error: ruleengine properties file [${properties_file}] not found."
	fi

	if [ ! -f "${properties_file_kafka_consumer}" ]
	then
		errorKafkaConsumerPropertiesFile=true
		echo "error: kafka consumer properties file [${properties_file_kafka_consumer}] not found."
	fi

	if [ ! -f "${properties_file_kafka_producer}" ]
	then
		errorKafkaProducerPropertiesFile=true
		echo "error: kafka producer properties file [${properties_file_kafka_producer}] not found."
	fi
